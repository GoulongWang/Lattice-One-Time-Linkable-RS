...
σ = α·κ·√(lN)（α=11 固定），使拒絕取樣常數 M₁, M₂ 在各組保持不變；α=11 在 N=1024
時恰好還原論文 Table 2 的 σ=31680。q = 2³²−99（質數、≡5 mod 8）三組共用；因每個 N
皆為 2 的次方，Lemma 1（X^N+1 partial splitting, d=2）對三組均成立。

| 參數集 | N | ⌈log₂q⌉ | h | l | v | k | κ | β | σ | M₁ | M₂ | E[attempts]=M₁M₂ | 安全(bits) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| lrs-light | 512 | 32 | 1 | 4 | 1 | 6 | 45 | 1 | 22401 | 2.989 | 3.828 | 11.44 | TBD |
| lrs-128 | 1024 | 32 | 1 | 4 | 1 | 6 | 45 | 1 | 31680 | 2.989 | 3.828 | 11.44 | TBD |
| lrs-192 | 2048 | 32 | 1 | 4 | 1 | 6 | 45 | 1 | 44802 | 2.989 | 3.828 | 11.44 | TBD |

所有組別均滿足正確性約束（q≡5 mod 8；M₁,M₂>1 且有限）。安全等級欄為 TBD：依規劃，
...
固定 lrs-128（N=1024），變動環大小 n。時間為中位數（ms），大小為 KB。
...
- **Sign 中位數非單調**（如 n=2 的 687 > n=4 的 363）：這是**取樣雜訊**，非真實趨勢。Sign 成本 =
  （拒絕取樣重試次數，幾何分布、平均 ~M₁M₂=11.4）×（每次約 2 趟 ring chain）。重試次數方差大、
...
**(1) 經驗正確性 100%（Table C1）**：對三組參數集做 Sign→Verify→Link 多次試驗，
誠實簽章全部通過、同簽章者全部 link、不同簽章者全部不 link。實證了 Theorem 1 的
bounded-norm 約束與參數選擇。

| 參數集 | n | 試驗數 | Verify | Link | Non-link | 全通過 | 重試平均(理論11.4) | 重試最大 |
|---|---|---|---|---|---|---|---|---|
| lrs-light | 4 | 20 | 20/20 | 19/19 | 19/19 | 是 | 11.4 | 41 |
| lrs-128 | 4 | 16 | 16/16 | 15/15 | 15/15 | 是 | 7.1 | 26 |
| lrs-192 | 2 | 9 | 9/9 | 8/8 | 8/8 | 是 | 13.6 | 33 |

（lrs-128/192 樣本較少使重試平均偏離 11.4；lrs-light 的 20 次平均恰為 11.4，與理論吻合。）

**(2) Sign vs Verify 成本差異**：Sign 比 Verify 貴約一個數量級，來源是 Lyubashevsky
**拒絕取樣**——平均要重跑整條 ring chain ~M₁M₂≈11 次才接受一次。對映 PrimeHE 中
「Mult 比 Add 貴 ~1000×」的同類觀察（成本來自方案結構而非實作）。

**(3) 瓶頸定位（Table C3）**：直接插樁量測各子程序佔比（lrs-128, n=8，單次簽章/驗章）：
...
python3 benchmark_full.py lrs-128 <n> <reps> [budget_s]   # Table B，每個 n 跑一次後合併
python3 correctness_gate.py <set> <n> <reps> [budget_s]    # Table C1（三組參數集）
python3 profile_bottleneck.py lrs-128 8      # Table C3
